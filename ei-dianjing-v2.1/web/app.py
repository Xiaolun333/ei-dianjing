...
import sys, os, json, uuid, threading, webbrowser, time, copy, io
...
def parse_discontinued_excel():
    """
    解析 EI Excel 的 DISCONTINUED sheet。
    Sheet 结构：
      第0行：标题行（COMPENDEX SOURCE LIST...）
      第1行：主表头（Source title, ISSN, EISSN, Publisher, FINAL COVERAGE, ...）
      第2行：子表头（nan, nan, nan, nan, Year, Volume, Issue, Pagination）
      第3行开始：实际数据（共约228条）
    返回 DataFrame 对象。
    """
    data_file = os.path.join(TOOLKIT_DIR, "data", "CPXSourceList_072026.xlsx")
    if not os.path.exists(data_file):
        return None, f"数据文件不存在: {data_file}"

    try:
        df = pd.read_excel(data_file, sheet_name='DISCONTINUED', header=None)
        # 实际数据从第3行开始（0-based index: row 0=标题, row 1=主表头, row 2=子表头, row 3+=数据）
        data_rows = df.iloc[3:].copy()
        data_rows.columns = ['source_title', 'issn', 'eissn', 'publisher', 'final_year', 'volume', 'issue', 'pagination']
        ...
        data_rows['final_year'] = data_rows['final_year'].apply(clean_year)

        # 清洗其他字段
        data_rows['source_title'] = data_rows['source_title'].fillna('').astype(str).str.strip()
        data_rows['issn'] = data_rows['issn'].fillna('').astype(str).str.strip()
        data_rows['eissn'] = data_rows['eissn'].fillna('').astype(str).str.strip()
        data_rows['publisher'] = data_rows['publisher'].fillna('').astype(str).str.strip()
        data_rows['volume'] = data_rows['volume'].fillna('').astype(str).str.strip()
        data_rows['issue'] = data_rows['issue'].fillna('').astype(str).str.strip()
        data_rows['pagination'] = data_rows['pagination'].fillna('').astype(str).str.strip()

        # 移除空行
        data_rows = data_rows[data_rows['source_title'] != ''].reset_index(drop=True)

        return data_rows, None
    except Exception as e:
        return None, f"解析停刊数据失败: {str(e)}"
...
/* 导航切换 */
.nav-tabs { display: flex; gap: 0; background: #fff; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); margin-bottom: 20px; overflow: hidden; }
.nav-tab { flex: 1; padding: 14px 24px; cursor: pointer; font-size: 14px; font-weight: 500; color: #666; text-align: center; border: none; background: transparent; transition: all 0.2s; }
.nav-tab:hover { background: #f5f7ff; }
.nav-tab.active { background: #1a237e; color: #fff; }

/* 停刊专用样式 */
.year-input { padding: 8px 14px; border: 1px solid #ddd; border-radius: 8px; width: 120px; font-size: 14px; }
.year-input:focus { outline: none; border-color: #1a237e; }
.discontinued-stat-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; margin-bottom: 20px; }
.discontinued-stat-grid .stat-card { border-left: 4px solid #f44336; }
.discontinued-stat-grid .stat-card .num { color: #f44336; }
</style>
</head>
<body>
...
<div class="navbar">
  <h1>EI 期刊智能筛选工具</h1>
  <span class="version">v2.1 — 新增停刊分析</span>
</div>

<div class="container">
  <!-- 导航切换 -->
  <div class="nav-tabs" id="navTabs">
    <div class="nav-tab active" onclick="switchMainTab('main')" id="navMain">📊 期刊筛选</div>
    <div class="nav-tab" onclick="switchMainTab('discontinued')" id="navDiscontinued">📉 停刊期刊分析</div>
  </div>
  ...
  <!-- ========== 停刊期刊分析界面 ========== -->
  <div id="discontinuedPanel" class="hidden">
    <!-- 停刊筛选面板 -->
    <div class="card">
      <div class="card-title"><span class="num" style="background:#f44336;">!</span> 停刊期刊筛选</div>

      <div class="filter-grid2">
        <div class="filter-section">
          <div class="filter-section-title">📅 终止年份范围</div>
          <div style="display:flex;gap:12px;align-items:center;">
            <div>
              <label style="font-size:12px;color:#888;display:block;margin-bottom:4px;">从</label>
              <input type="number" class="year-input" id="dcYearFrom" placeholder="起始年份" min="1900" max="2100">
            </div>
            <span style="color:#999;">~</span>
            <div>
              <label style="font-size:12px;color:#888;display:block;margin-bottom:4px;">到</label>
              <input type="number" class="year-input" id="dcYearTo" placeholder="结束年份" min="1900" max="2100">
            </div>
          </div>
        </div>

        <div class="filter-section">
          <div class="filter-section-title">🔍 标题搜索</div>
          <input type="text" class="search-box" id="dcTitleSearch" placeholder="输入期刊标题关键词..." style="width:100%;">
        </div>

        <div class="filter-section">
          <div class="filter-section-title">🔍 ISSN 搜索</div>
          <input type="text" class="search-box" id="dcIssnSearch" placeholder="输入 ISSN 或 EISSN..." style="width:100%;">
        </div>

        <div class="filter-section">
          <div class="filter-section-title">🏢 出版商</div>
          <div class="multi-select" id="msDcPublisher">
            <div class="ms-display" onclick="toggleDropdown('msDcPublisher')">
              <span class="ms-placeholder">点击选择出版商（可多选）...</span>
              <span class="ms-arrow">▼</span>
            </div>
            <div class="ms-dropdown">
              <input class="ms-search" placeholder="搜索出版商..." oninput="filterDropdownOptions(this, 'msDcPublisher')">
              <div class="ms-options" id="dcPublisherOptions"></div>
              <div class="ms-footer">
                <a onclick="selectAllDropdown('msDcPublisher', true)">全选</a>
                <a onclick="selectAllDropdown('msDcPublisher', false)">清空</a>
              </div>
            </div>
          </div>
        </div>
      ...
      <div style="display:flex;justify-content:flex-end;gap:12px;margin-top:20px;padding-top:16px;border-top:2px solid #e8eaf6;">
        <button class="btn btn-outline" onclick="resetDiscontinuedFilter()">🔄 重置</button>
        <button class="btn btn-primary" id="btnDcFilter" onclick="runDiscontinuedFilter()">🔍 执行筛选</button>
      </div>

      <div class="loading" id="dcLoading">
        <div class="spinner"></div>
        <p>正在加载停刊数据...</p>
      </div>
    </div>
    ...
    <!-- 停刊统计卡片 -->
    <div class="discontinued-stat-grid" id="dcStatsGrid">
      <div class="stat-card">
        <div class="num" id="dcStatTotal">-</div>
        <div class="label">停刊期刊总数</div>
      </div>
      <div class="stat-card">
        <div class="num" id="dcStatPublishers">-</div>
        <div class="label">涉及出版商数</div>
      </div>
      <div class="stat-card">
        <div class="num" id="dcStatEarliest">-</div>
        <div class="label">最早停刊年份</div>
      </div>
      <div class="stat-card">
        <div class="num" id="dcStatLatest">-</div>
        <div class="label">最晚停刊年份</div>
      </div>
    </div>
    ...
    <div class="card">
      <div class="card-title">
        <span class="num" style="background:#f44336;">!</span> 停刊结果
        <span style="margin-left:auto;display:flex;gap:8px;">
          <input type="text" class="search-box" id="dcTableSearch" placeholder="🔍 输入关键词后点击搜索...">
          <button class="btn btn-outline btn-sm" onclick="filterDcTable()">搜索</button>
          <button class="btn btn-success btn-sm" onclick="exportDiscontinuedExcel()">📥 导出 Excel</button>
        </span>
      </div>

      <div class="tabs">
        <div class="tab active" onclick="switchDcTab(event,'dcTable')">数据表格</div>
        <div class="tab" onclick="switchDcTab(event,'dcDist')">年份分布</div>
      </div>

      <div id="view-dc-table">
        <div class="table-wrapper">
          <table id="dcResultTable">
            <thead><tr id="dcTableHead"></tr></thead>
            <tbody id="dcTableBody"></tbody>
          </table>
        </div>
        <div style="margin-top:8px;font-size:13px;color:#888;" id="dcTableInfo"></div>
      </div>

      <div id="view-dc-dist" class="hidden">
        <h3 style="margin-bottom:12px;">停刊年份分布</h3>
        <div id="dcDistYear"></div>
      </div>
    </div>
  </div>
</div>
...
function switchMainTab(tab) {
  ...
  document.getElementById('mainPanel').classList.toggle('hidden', tab !== 'main');
  document.getElementById('discontinuedPanel').classList.toggle('hidden', tab !== 'discontinued');

  if (tab === 'main') {
    document.getElementById('navMain').classList.add('active');
  } else {
    document.getElementById('navDiscontinued').classList.add('active');
    if (!discontinuedData) {
      loadDiscontinuedData();
    }
  }
}
...
function loadDiscontinuedData() {
  document.getElementById('dcLoading').classList.add('show');
  fetch('/api/discontinued', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) })
    .then(r => r.json())
    .then(data => {
      if (data.error) { showToast(data.error, 'error'); return; }
      discontinuedData = data;
      discontinuedFiltered = data.rows;

      // 初始化出版商下拉
      const publishers = data.publishers || [];
      initMultiSelect('msDcPublisher', publishers.map(p => ({ value: p, label: p })));

      // 填充统计
      document.getElementById('dcStatTotal').textContent = data.stats.total;
      document.getElementById('dcStatPublishers').textContent = data.stats.publishers;
      document.getElementById('dcStatEarliest').textContent = data.stats.earliest_year || '-';
      document.getElementById('dcStatLatest').textContent = data.stats.latest_year || '-';

      // 渲染表格和分布
      renderDiscontinuedTable(data.rows);
      renderDiscontinuedDist(data.dist);

      showToast('停刊数据加载完成！', 'success');
    })
    .catch(err => showToast('加载停刊数据失败: ' + err, 'error'))
    .finally(() => {
      document.getElementById('dcLoading').classList.remove('show');
    });
}

function runDiscontinuedFilter() {
  if (!discontinuedData) return;
  const btn = document.getElementById('btnDcFilter');
  btn.disabled = true;

  const yearFrom = document.getElementById('dcYearFrom').value;
  const yearTo = document.getElementById('dcYearTo').value;
  const title = document.getElementById('dcTitleSearch').value;
  const issn = document.getElementById('dcIssnSearch').value;
  const publishers = getSelectedValues('msDcPublisher');

  fetch('/api/discontinued', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
  ...
  .then(data => {
    ...
    renderDiscontinuedTable(data.rows);
    renderDiscontinuedDist(data.dist);
    showToast(`筛选完成: ${data.rows.length} 条结果`, 'success');
  })
  .catch(err => showToast('筛选失败: ' + err, 'error'))
  .finally(() => { btn.disabled = false; });
}

function resetDiscontinuedFilter() {
  document.getElementById('dcYearFrom').value = '';
  document.getElementById('dcYearTo').value = '';
  document.getElementById('dcTitleSearch').value = '';
  document.getElementById('dcIssnSearch').value = '';
  document.querySelectorAll('#msDcPublisher .ms-option input').forEach(cb => cb.checked = false);
  updateMultiSelectDisplay('msDcPublisher');
  if (discontinuedData) {
    discontinuedFiltered = discontinuedData.rows;
    renderDiscontinuedTable(discontinuedData.rows);
    renderDiscontinuedDist(discontinuedData.dist);
  }
}

function renderDiscontinuedTable(rows) {
  const head = document.getElementById('dcTableHead');
  const body = document.getElementById('dcTableBody');
  head.innerHTML = '';
  body.innerHTML = '';

  const columns = ['Source title', 'Publisher', 'Final Year', 'Final Volume', 'Final Issue', 'ISSN', 'EISSN'];
  columns.forEach(col => {
    const th = document.createElement('th');
    th.textContent = col;
    head.appendChild(th);
  ...
  displayRows.forEach(row => {
    ...
    cells.forEach((val, idx) => {
      const td = document.createElement('td');
      if (idx === 0) {
        const searchUrl = 'https://www.google.com/search?q=' + encodeURIComponent(row.source_title);
        td.innerHTML = `<a class="title-link" href="${searchUrl}" target="_blank">${val || ''}</a>`;
      } else {
        td.textContent = val || '';
      }
      tr.appendChild(td);
  ...
  document.getElementById('dcTableInfo').textContent =
    `共 ${rows.length} 条 | 显示前 ${Math.min(rows.length, 200)} 条（导出包含全部）`;
}

function renderDiscontinuedDist(dist) {
  if (!dist || !dist.years) return;
  const arr = dist.years;
  const maxVal = Math.max(...arr.map(d => d[1]), 1);
  const el = document.getElementById('dcDistYear');
  el.innerHTML = arr.map(item => {
    const [name, count] = item;
    const pct = (count / maxVal * 100).toFixed(1);
    return `<div class="dist-row">
      <div class="dist-label" style="width:60px;text-align:right;">${name}</div>
      <div class="dist-bar" style="width:${pct}%;background:#f44336;"></div>
      <div class="dist-count">${count}</div>
    </div>`;
  }).join('');
}

function switchDcTab(e, tab) {
  document.querySelectorAll('#discontinuedPanel .tab').forEach(t => t.classList.remove('active'));
  e.currentTarget.classList.add('active');
  document.getElementById('view-dc-table').classList.toggle('hidden', tab !== 'dcTable');
  document.getElementById('view-dc-dist').classList.toggle('hidden', tab !== 'dcDist');
}

function filterDcTable() {
  const q = document.getElementById('dcTableSearch').value.toLowerCase();
  if (!q) {
    renderDiscontinuedTable(discontinuedFiltered);
    return;
  }
  const filtered = discontinuedFiltered.filter(row => {
    const text = (row.source_title + ' ' + row.publisher + ' ' + row.issn + ' ' + row.eissn).toLowerCase();
    return text.includes(q);
  });
  renderDiscontinuedTable(filtered);
}

function exportDiscontinuedExcel() {
  showToast('正在生成停刊 Excel...', 'info');
  window.location.href = '/api/discontinued_export';
}
...
@app.route('/api/discontinued', methods=['POST'])
def api_discontinued():
    """
    解析停刊期刊数据，支持筛选。
    POST 参数：
      year_from: 起始年份（可选）
      year_to: 结束年份（可选）
      title: 标题关键词（可选，模糊匹配）
      issn: ISSN/EISSN 搜索（可选，模糊匹配）
      publishers: 出版商列表（可选，精确匹配）
    """
    try:
        df, error = parse_discontinued_excel()
        if error:
        ...
        if year_from is not None:
            filtered = filtered[filtered['final_year'] >= year_from]
        if year_to is not None:
            filtered = filtered[filtered['final_year'] <= year_to]
        if title_q:
            title_q = title_q.lower()
            filtered = filtered[filtered['source_title'].str.lower().str.contains(title_q, na=False)]
        if issn_q:
            issn_q = issn_q.lower()
            filtered = filtered[
                filtered['issn'].str.lower().str.contains(issn_q, na=False) |
                filtered['eissn'].str.lower().str.contains(issn_q, na=False)
            ]
        if publishers and len(publishers) > 0:
            filtered = filtered[filtered['publisher'].isin(publishers)]

        # 按终止年份降序排列（最新的停刊在前）
        filtered = filtered.sort_values(by='final_year', ascending=False, na_position='last')
        ...
        for row in rows:
            # 确保年份为整数或 None
            if pd.notna(row.get('final_year')):
                row['final_year'] = int(row['final_year'])
            else:
                row['final_year'] = None

        # 统计信息
        all_years = df['final_year'].dropna().astype(int)
        filtered_years = filtered['final_year'].dropna().astype(int)

        stats = {
            "total": len(df),
            "filtered": len(filtered),
            "publishers": len(df['publisher'].unique()),
            "earliest_year": int(all_years.min()) if len(all_years) > 0 else None,
            "latest_year": int(all_years.max()) if len(all_years) > 0 else None,
        }

        # 年份分布（全部数据的分布）
        year_counts = df['final_year'].value_counts().sort_index(ascending=False)
        dist_years = [[str(int(y)), int(c)] for y, c in year_counts.items() if pd.notna(y)]

        # 出版商列表
        publisher_list = sorted(df['publisher'].dropna().unique().tolist())

        return jsonify({
            "rows": rows,
            "stats": stats,
            "dist": {"years": dist_years},
            "publishers": publisher_list,
        })
    except Exception as e:
        return jsonify({"error": f"停刊数据查询失败: {str(e)}"}), 500


@app.route('/api/discontinued_export')
def api_discontinued_export():
    """导出停刊期刊为 Excel 文件。"""
    try:
        df, error = parse_discontinued_excel()
        if error:
        ...
        output_dir = os.path.join(TOOLKIT_DIR, "data", "output")
        os.makedirs(output_dir, exist_ok=True)
        output_path = os.path.join(output_dir, f"EI_Discontinued_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx")

        # 准备输出列
        export_df = df[['source_title', 'publisher', 'final_year', 'volume', 'issue', 'issn', 'eissn', 'pagination']].copy()
        export_df.columns = ['Source title', 'Publisher', 'Final Year', 'Final Volume', 'Final Issue', 'ISSN', 'EISSN', 'Pagination']

        # 使用 openpyxl 写入并添加简单样式
        with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
            export_df.to_excel(writer, index=False, sheet_name='Discontinued')
            worksheet = writer.sheets['Discontinued']
